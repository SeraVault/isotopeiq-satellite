...
# Commands that occasionally fail transiently (launchd/dbus-style hiccups,
# busy resource) get one retry before we give up and log a warning.
...
def main():
    ...
    if not is_root() and not _has_passwordless_sudo():
        sys.stderr.write(
            ...
            'Some data (FileVault status, SIP status, keychain certs, full '
